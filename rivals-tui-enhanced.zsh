...
draw_pattern_creator() {
    ...
    local icons=("🎨" "🌟" "💎" "🔥" "❄️" "⚡" "🌈" "🌸" "🍀" "🎭" "🎪" "🎯")
    echo -e "${BORDER}${V}${RESET}  ${icon_highlight}${TEXT}Icon:${RESET} "
    ...
    echo -e "${BORDER}${V}${RESET}  ${TEXT}Current Pattern:${RESET}$(printf ' %.0s' {1..50})${BORDER}${V}${RESET}"
    ...
    if [[ "$CREATOR_MODE" == "name" ]]; then
        echo -e "${BORDER}${V}${RESET}  ${DIM}Type pattern name, ${TEXT}Tab${RESET}${DIM} to continue${RESET}$(printf ' %.0s' {1..31})${BORDER}${V}${RESET}"
    elif [[ "$CREATOR_MODE" == "icon" ]]; then
        echo -e "${BORDER}${V}${RESET}  ${DIM}Arrow keys to select, ${TEXT}Space${RESET}${DIM} to choose, ${TEXT}Tab${RESET}${DIM} to continue${RESET}$(printf ' %.0s' {1..10})${BORDER}${V}${RESET}"
        ...
        "icon")
            case "$char" in
                $'\t') # Tab - move to color selection
                    ...
                    ;;
                $'\x1b') # Escape sequences for arrow keys
